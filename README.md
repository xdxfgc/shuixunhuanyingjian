...
`添加传感器方案/` 目录下有 9 份文档：一份当前**引脚与接口总表**，加上 7 份"如何再加一路模块"的操作步骤，
以及一份"更换 WiFi 网络"的排查指南。
每份都包含引脚选择、接线、完整模块代码、config.h / 主文件 / web_server.cpp 的改动位置、测试方法和常见坑。

| 文档 | 加什么 | 新增接口 |
|------|--------|----------|------|
| `00_引脚与接口总表.md` | **当前所有引脚 + 全部 API 接口** + 通用套路 | — | 现状 |
| `增加第二路流量传感器-操作步骤.md` | 流量计 #2（**D19**） | `/api/flow2` 等 3 个 | ✅ 已实现 |
| `03_增加第二路超声波水位-操作步骤.md` | 超声波 #2（**D4/D13**） | `/api/level2`、`/api/level2/height` | ✅ 已实现 |
| `01_增加第三路温度传感器-操作步骤.md` | DS18B20 #3（D5） | `/api/temperature3` | 待实现 |
| `02_增加第二路压力传感器-操作步骤.md` | 压力传感器 #2（D35） | `/api/pressure2` | 待实现 |
| `04_增加第二路光照传感器-操作步骤.md` | GY-302 #2（共用 I2C） | `/api/light2` | 待实现 |
| `05_增加第三路继电器-操作步骤.md` | 继电器 #3（D17） | `/api/relay3/on` 等 4 个 | 待实现 |
| `06_增加第三路加热继电器-操作步骤.md` | 加热继电器 #3（D17） | `/api/heater3/on` 等 4 个 | 待实现 |
| `07_更换WiFi网络-操作步骤.md` | 换网络时改什么、怎么排查 | — | 指南 |

> ⚠️ 05 和 06 **都要用 D17**，而现在只剩这一个空闲双向脚，**只能二选一**；
> 两个都要加就得用 PCF8574 I2C 扩展板。
